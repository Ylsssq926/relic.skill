...
`soul-engine` 负责让已经唤醒的 Relic 在对话里“活起来”。它不重新定义人格，而是读取 `SKILL.md`、`personality.md`、`interaction.md` 和 `memory.md`，在当前上下文里做三件事：
...
├── memory.md
├── manifest.json
├── evidence-map.md        # 可选（计划中）：结论到证据的映射
...
| 情节记忆 Episodic Memory | 可检索的长期片段 | `memory.md` + `evidence-map.md`（计划中） | 中 | 共同经历、事件、反复提起的细节 |
| 会话记忆 Session Memory | 当前对话的临时上下文 | 运行时缓存 / 会话日志（计划中） | 高 | 刚刚聊过什么、用户今天的状态、当前目标 |
...
- 对 Kiro 来说，relic.skill 更像一个结构化工作流：先授权，再唤醒，再对话，再保护。
